function [dots, pmin] = sdd_detect_dots_2D(dotBars, optics, dotMargin, pmin, lengthLims, imageNumber, imageName, actions, bgPixels,tiles)
    % sdd_detect_dots_2D
    %
    % Detects dots on a 2D mask instead of detecting them along a straight
    % line. More robust in case the molecule has slight bends.
  ...
  for i = 1:numel(dotBars)
   %%
      M = dotBars{i};
        ...
        Pd = imdilate(M,ones(3,3));
        local_maxima = (M == Pd);
        local_maxima(mask2) = 0;
%         nexttile
%         imagesc(local_maxima)

        [posY,posX] = find(local_maxima==1);

        
        local_maxima_filt = local_maxima;
        for i2=1:length(posY)
            if M(posY(i2),posX(i2))< pmin
                local_maxima_filt(posY(i2),posX(i2))= 0;
            end
        end
%         M(M==)=nan;
        [posY2,posX2] = find(local_maxima_filt==1);

%         figure
%         tiledlayout(1,2);
% %         imagesc(local_maxima)
%         nexttile,imagesc(max(M(:))-M)
%      
%         hold on
%         plot(posX,posY,'redx')
%         colormap(gray)
