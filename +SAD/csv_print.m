function printName = csv_print(output, sets, runNo, i, filtered)
...
%   fclose(fid);
  ...
  barLength = zeros(1,length(output{i}.dots));
...
function printName = print_version(nImage, experiment, firstName, runNo,i, filtered)
  ...
  version = runNo;
  printName = fullfile(folderName, [nameType, num2str(version), '_', num2str(i),'.xlsx']);
end
