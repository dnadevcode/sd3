function output = dnarec_skel(target, inputsets)
  ...
  if nargin < 2
    ...
    experiment.lowLim = exp(5); % Set the low score threshold to consider a region "signal" (very important)
  ...
  functions.bc_extract = @(movies, optics, lengthLims, imageNumber, saveName, bgPixels) extract_barcodes(movies, optics, lengthLims, imageNumber, saveName, bgPixels, sets, experiment, actions);
